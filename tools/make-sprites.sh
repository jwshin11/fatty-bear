#!/usr/bin/env bash
# Turn gifs/bubu/*.gif, gifs/dudu/*.gif and the fudu/ stills into sprite sheets
# under public/sprites/.
...
# One directory per character, so the height comes from the folder rather than
# from a filename prefix -- a gif keeps whatever name it arrived with.
for gif in gifs/bubu/*.gif gifs/dudu/*.gif; do
  [ -e "$gif" ] || continue
  name=$(basename "$gif" .gif)
  case "$(basename "$(dirname "$gif")")" in
    dudu) height=$DUDU_H ;;
    *)    height=$BUBU_H ;;
  esac
